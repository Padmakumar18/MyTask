...
import { WEEKEND_DAYS, Weekday } from '../../models/weekday.model';
import { UserService } from '../../services/user.service';
import { toast } from 'ngx-sonner';

@Component({
  selector: 'app-weekend-days',
  imports: [CommonModule, ReactiveFormsModule],
  templateUrl: './weekend-days.html',
  styleUrl: './weekend-days.css',
})
export class WeekendDays implements OnInit {
  ...
  // Day selection
  selectedWeekday = signal<number>(6); // Default to Saturday
  ...
  selectedDayName = computed(() => {
    ...
    return day?.day_name || 'Saturday';
  });

  constructor() {
    this.taskForm = this.fb.group({
      weekday_id: [6, [Validators.required]],
  ...
  async ngOnInit() {
    ...
    if (userId) {
      console.log('Weekend Days - User ID loaded:', userId);
      await this.loadTasks();
    } else {
      console.warn('Weekend Days - No user found');
      toast.warning('Please log in to view your weekend tasks');
    }
  }
  ...
  async loadTasks() {
    ...
    try {
      // Load tasks for Saturday and Sunday (weekday_id 6-7)
      const tasks = await this.weekdayTaskService.getTasksByWeekdayRange(userId, 6, 7);
